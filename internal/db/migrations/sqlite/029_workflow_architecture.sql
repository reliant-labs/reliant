...
-- Add node filtering columns to messages
ALTER TABLE messages ADD COLUMN node_id TEXT NOT NULL DEFAULT '0';
ALTER TABLE messages ADD COLUMN node_path TEXT NOT NULL DEFAULT '0';
CREATE INDEX idx_messages_node ON messages(chat_id, node_id, ordinal);

-- Add node filtering columns to message_content_blocks
ALTER TABLE message_content_blocks ADD COLUMN node_id TEXT NOT NULL DEFAULT '0';
ALTER TABLE message_content_blocks ADD COLUMN node_path TEXT NOT NULL DEFAULT '0';
...
CREATE INDEX idx_tool_calls_chat ON tool_calls(chat_id, status);
CREATE INDEX idx_tool_calls_node ON tool_calls(chat_id, node_id, status);
...
CREATE INDEX idx_workflow_exec_chat ON workflow_executions(chat_id, status);
CREATE INDEX idx_workflow_exec_parent ON workflow_executions(parent_execution_id);
