-- +goose Up
-- Migration: Add run_output update type to chat_updates
-- This allows workflow run steps to emit output to the UI without requiring LLM context
...
-- +goose Down
-- Revert: Remove run_output from allowed update types
-- Note: This will fail if there are any run_output records in the table

-- Step 1: Drop trigger
DROP TRIGGER IF EXISTS chat_updates_chat_update;

-- Step 2: Create table without run_output
