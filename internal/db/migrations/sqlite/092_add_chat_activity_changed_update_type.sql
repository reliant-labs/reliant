-- +goose Up
-- Add 'chat_activity_changed' to user_updates CHECK constraint for workflow status notifications

-- First drop the trigger that references user_updates
DROP TRIGGER IF EXISTS user_updates_chat_config_update;

-- Recreate user_updates table with updated CHECK constraint
...
-- Remove 'chat_activity_changed' from CHECK constraint
...
SELECT id, user_id, sequence_number, project_id, worktree_id, chat_id, update_type, entity_type, entity_id, data, created_at
FROM user_updates
WHERE update_type != 'chat_activity_changed';
