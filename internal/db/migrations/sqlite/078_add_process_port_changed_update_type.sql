-- +goose Up
-- Add 'process_port_changed' to user_updates CHECK constraint for real-time port detection

-- Drop the trigger first (it references user_updates)
DROP TRIGGER IF EXISTS user_updates_chat_config_update;

-- Recreate user_updates table with 'process_port_changed' in CHECK constraint
...
-- Recreate trigger from migration 071
...
-- Recreate user_updates table without 'process_port_changed'
...
-- Copy data (excluding any process_port_changed entries)
...
SELECT id, user_id, sequence_number, project_id, worktree_id, chat_id, update_type, entity_type, entity_id, data, created_at
FROM user_updates
WHERE update_type != 'process_port_changed';
