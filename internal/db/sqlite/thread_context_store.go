...
type threadStore struct{ q sqlitedb.Querier }

// NewThreadStore creates the SQLite thread store implementation.
func NewThreadStore(q sqlitedb.Querier) core.ThreadStore { return &threadStore{q: q} }
...
func (s *threadStore) CreateThread(ctx context.Context, thread *core.Thread) (*core.Thread, error) {
	...
	return threadFromSQLc(result), nil
}
...
func (s *threadStore) ListThreadsByConversation(ctx context.Context, conversationID string) ([]*core.Thread, error) {
	...
	return threadsFromSQLc(results), nil
}
...
func (s *threadStore) UpdateThreadWorkflow(ctx context.Context, threadID, workflowID string) (*core.Thread, error) {
	result, err := s.q.UpdateThreadWorkflow(ctx, sqlitedb.UpdateThreadWorkflowParams{WorkflowID: threadStringToNullString(workflowID), ID: threadID})
	if err != nil {
...
func (s *threadStore) UpdateThreadForkPoint(ctx context.Context, threadID string, forkAtOrdinal *int64, forkAtContextWindowID *string) (*core.Thread, error) {
	result, err := s.q.UpdateThreadForkPoint(ctx, sqlitedb.UpdateThreadForkPointParams{
...
type contextWindowStore struct{ q sqlitedb.Querier }

// NewContextWindowStore creates the SQLite context-window store implementation.
func NewContextWindowStore(q sqlitedb.Querier) core.ContextWindowStore {
...
func (s *contextWindowStore) CreateContextWindow(ctx context.Context, cw *core.ContextWindow) (*core.ContextWindow, error) {
	...
	return contextWindowFromSQLc(result), nil
}
...
func (s *contextWindowStore) GetContextWindowBySequence(ctx context.Context, threadID string, sequence int) (*core.ContextWindow, error) {
	result, err := s.q.GetContextWindowBySequence(ctx, sqlitedb.GetContextWindowBySequenceParams{ThreadID: threadID, Sequence: int64(sequence)})
	if err != nil {
...
func (s *contextWindowStore) ListContextWindowsByThread(ctx context.Context, threadID string) ([]*core.ContextWindow, error) {
	...
	return contextWindowsFromSQLc(results), nil
}
...
func (s *contextWindowStore) SetCompactionSummaryMessage(ctx context.Context, cwID, messageID string) (*core.ContextWindow, error) {
	result, err := s.q.SetCompactionSummaryMessage(ctx, sqlitedb.SetCompactionSummaryMessageParams{
...
func threadFromSQLc(st sqlitedb.Thread) *core.Thread {
	return &core.Thread{
		ID:                    st.ID,
...
func threadsFromSQLc(rows []sqlitedb.Thread) []*core.Thread {
	threads := make([]*core.Thread, len(rows))
	for i, row := range rows {
		threads[i] = threadFromSQLc(row)
	}
	return threads
}

func threadToCreateParams(t *core.Thread) sqlitedb.CreateThreadParams {
	return sqlitedb.CreateThreadParams{
		ID:                    t.ID,
...
func contextWindowFromSQLc(cw sqlitedb.ContextWindow) *core.ContextWindow {
...
func contextWindowsFromSQLc(rows []sqlitedb.ContextWindow) []*core.ContextWindow {
	items := make([]*core.ContextWindow, len(rows))
	for i, row := range rows {
		items[i] = contextWindowFromSQLc(row)
	}
	return items
}

func contextWindowToCreateParams(cw *core.ContextWindow) sqlitedb.CreateContextWindowParams {
	return sqlitedb.CreateContextWindowParams{
		ID:                         cw.ID,
		ThreadID:                   cw.ThreadID,
