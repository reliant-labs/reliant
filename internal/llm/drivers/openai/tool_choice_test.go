...
func TestPreparedParams_PinsToolChoiceWhenForcedAndPresent(t *testing.T) {
	client := &OpenaiClient{Options: llm.DriverOptions{ForceToolChoice: "set_title"}}
...
func TestPreparedParams_OmitsToolChoiceWhenUnset(t *testing.T) {
	client := &OpenaiClient{Options: llm.DriverOptions{}}
...
func TestSendResponsesParams_PinsToolChoiceWhenForcedAndPresent(t *testing.T) {
	client := &OpenaiClient{Options: llm.DriverOptions{ForceToolChoice: "set_title"}}

	convertedTools := client.convertToolsToResponsesTools([]llmtools.Tool{schemaOnlyTool("set_title")})

	if !responsesToolListHasFunction(convertedTools, "set_title") {
		t.Fatalf("expected converted tools to contain set_title")
	}
	if responsesToolListHasFunction(convertedTools, "other_tool") {
		t.Fatalf("did not expect converted tools to contain other_tool")
	}
}
