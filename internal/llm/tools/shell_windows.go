...
// executeShellCommand runs a command using PowerShell on Windows
func executeShellCommand(rctx *rctx.ToolContext, command string, workingDir string, timeoutMs int, env map[string]string) shellCommandResult {
	...
	// Use PowerShell with -NoProfile to avoid loading user profile scripts
	// -Command executes the command and exits
	cmd := exec.CommandContext(ctx, "powershell.exe", "-NoProfile", "-Command", command)
	cmd.Dir = workingDir

	// Set up environment variables
	if env != nil && len(env) > 0 {
		cmd.Env = append(cmd.Environ(), formatShellEnvVars(env)...)
	}
