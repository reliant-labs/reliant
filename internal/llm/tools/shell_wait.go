...
type ShellWaitParams struct {
	...
	// with the exit code, so the common case needs no follow-up shell_output.
	TailLines int `json:"tail_lines,omitempty" jsonschema:"description=Lines of output to return when the process exits (default: 50, 0 for none). Use shell_output for the full log or for regex filtering."`
}

type ShellWaitResponseMetadata struct {
...
const (
	ShellWaitToolName = "shell_wait"

	// shellWaitDefaultTimeout is the default block budget.
	//
	...
	// which is the polling cost shell_wait was built to remove.
	shellWaitDefaultTimeout = MaxBlockingToolWait
	shellWaitMaxTimeout     = MaxBlockingToolWait

	// shellWaitPollInterval trades responsiveness against daemon chatter. The
	// wait is server-side, so this costs no model round-trips: it is a loop in
	// one tool call, not one call per poll.
	shellWaitPollInterval = 500 * time.Millisecond

	shellWaitDefaultTailLines = 50

	shellWaitDescription = `Block until a background process exits, then return its exit code and recent output.
...
func NewShellWaitTool() Tool {
	tool := &shellWaitTool{}
	return NewToolWrapper[ShellWaitParams, ToolResponse](tool)
}

func (b *shellWaitTool) Name() string {
	return ShellWaitToolName
}

func (b *shellWaitTool) Description() string {
	return shellWaitDescription
}

func (b *shellWaitTool) RequiresPermission(params ShellWaitParams) (bool, error) {
	// Waiting observes a process that is already running; it starts nothing and
	// changes nothing. Read-only, like shell_output.
	return false, nil
}

func (b *shellWaitTool) Execute(rctx *rctx.ToolContext, params ShellWaitParams) (ToolResponse, error) {
	if rctx.Daemon == nil {
	...
	budget := shellWaitDefaultTimeout
	if params.TimeoutSeconds > 0 {
		budget = time.Duration(params.TimeoutSeconds) * time.Second
		if budget > shellWaitMaxTimeout {
			...
			budget = shellWaitMaxTimeout
		}
	}

	tailLines := shellWaitDefaultTailLines
	...
	if proc == nil {
		return NewTextErrorResponse(fmt.Sprintf(
			"No process with ID %q. Use shell_list to see processes in this workspace.", params.ProcessID)), nil
	}
	...
	for {
		...
		case <-rctx.Done():
			return b.stillRunningResponse(proc, time.Since(start))
		case <-time.After(shellWaitPollInterval):
		}
...
func (b *shellWaitTool) exitedResponse(rctx *rctx.ToolContext, proc *daemon.ProcessInfo, tailLines int, waited time.Duration) (ToolResponse, error) {
	exitCodeStr := "unknown"
	...
	metadata := ShellWaitResponseMetadata{
		ProcessID: proc.ID,
		Status:    proc.Status,
...
func (b *shellWaitTool) stillRunningResponse(proc *daemon.ProcessInfo, waited time.Duration) (ToolResponse, error) {
	output := fmt.Sprintf(
		"Process %s is STILL RUNNING after %s — not an error, and it has not been killed.\nCommand: %s\n\nCall shell_wait again to keep waiting, or shell_output to see progress so far.",
...
func (b *shellWaitTool) vanishedResponse(proc *daemon.ProcessInfo, waited time.Duration) (ToolResponse, error) {
	output := fmt.Sprintf(
