...
// shell_wait must reach the model through the same tag filters as the rest of
...
func TestShellWait_IsOfferedWithTheShellFamily(t *testing.T) {
	...
	for _, filter := range []string{"tag:default", "tag:shell", "tag:plan", "tag:readonly"} {
		...
		if !found {
			t.Errorf("shell_wait missing from %s — it must travel with shell_output/shell_list", filter)
		}
	}
}
