...
type SourcegraphParams struct {
	Query         string `json:"query" jsonschema:"required,description=The Sourcegraph search query"`
	Count         int    `json:"count,omitempty" jsonschema:"description=Optional number of results to return (default: 10 max: 20)"`
	ContextWindow int    `json:"context_window,omitempty" jsonschema:"description=Number of lines of context to show around matches (default: 3)"`
	Timeout       int    `json:"timeout,omitempty" jsonschema:"description=Optional timeout in seconds (max 60)"`
}
...
func (t *sourcegraphTool) Description() string {
	return sourcegraphToolDescription
}

func (t *sourcegraphTool) RequiresPermission(params SourcegraphParams) (bool, error) {
	// sourcegraph tool requires permissions as it accesses external services
	return true, nil
}

func (t *sourcegraphTool) Execute(rctx *rctx.ToolContext, params SourcegraphParams) (ToolResponse, error) {
