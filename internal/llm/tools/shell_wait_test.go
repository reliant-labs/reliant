...
func runWait(t *testing.T, d daemon.Client, params ShellWaitParams) (ToolResponse, ShellWaitResponseMetadata) {
	t.Helper()
	resp, err := (&shellWaitTool{}).Execute(waitCtx(t, d), params)
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	var meta ShellWaitResponseMetadata
	if resp.Metadata != "" {
...
func TestShellWait_ReturnsExitCodeWhenProcessExits(t *testing.T) {
	t.Parallel()
	d := &scriptedDaemon{
	...
	resp, meta := runWait(t, d, ShellWaitParams{ProcessID: "p1"})
	...
	// output. Returning it here is what removes the follow-up shell_output call.
...
func TestShellWait_TimeoutIsNotAnError(t *testing.T) {
	t.Parallel()
	d := &scriptedDaemon{
	...
	resp, meta := runWait(t, d, ShellWaitParams{ProcessID: "p1", TimeoutSeconds: 1})
	...
	if !strings.Contains(resp.Content, "shell_wait again") {
...
func TestShellWait_UnknownProcessFailsFast(t *testing.T) {
	t.Parallel()
	d := &scriptedDaemon{steps: []*daemon.ProcessInfo{nil}, listErrAfter: -1}

	start := time.Now()
	resp, err := (&shellWaitTool{}).Execute(waitCtx(t, d), ShellWaitParams{ProcessID: "nope"})
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	...
	if !strings.Contains(resp.Content, "shell_list") {
...
func TestShellWait_ClampsTimeoutBelowToolCeiling(t *testing.T) {
	t.Parallel()
	d := &scriptedDaemon{steps: []*daemon.ProcessInfo{exited("p1", 0)}, listErrAfter: -1}

	// Ask for an hour; the call must still return promptly.
	_, meta := runWait(t, d, ShellWaitParams{ProcessID: "p1", TimeoutSeconds: 3600})
	if !meta.HasExited {
	...
	if shellWaitMaxTimeout != MaxBlockingToolWait {
...
func TestShellWait_SurvivesTransientListingFailure(t *testing.T) {
	t.Parallel()
	d := &scriptedDaemon{
	...
	_, meta := runWait(t, d, ShellWaitParams{ProcessID: "p1", TimeoutSeconds: 10})
...
func TestShellWait_StopsWhenToolCallCancelled(t *testing.T) {
	t.Parallel()
	...
	start := time.Now()
	resp, err := (&shellWaitTool{}).Execute(tc, ShellWaitParams{ProcessID: "p1", TimeoutSeconds: 240})
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	elapsed := time.Since(start)
