...
type ShellOutputResponseMetadata struct {
...
const (
	ShellOutputToolName    = "shell_output"
	shellOutputDescription = `Retrieves output from a background process with pagination and regex filtering support.
...
func NewShellOutputTool() Tool {
	tool := &shellOutputTool{}
	return NewToolWrapper[ShellOutputParams, ToolResponse](tool)
}

func (b *shellOutputTool) Name() string {
	return ShellOutputToolName
}

func (b *shellOutputTool) Description() string {
	return shellOutputDescription
}

func (b *shellOutputTool) RequiresPermission(params ShellOutputParams) (bool, error) {
	// shell_output tool doesn't require permissions as it's read-only
	return false, nil
}

func (b *shellOutputTool) Execute(rctx *rctx.ToolContext, params ShellOutputParams) (ToolResponse, error) {
	if rctx.Daemon == nil {
	...
	metadata := ShellOutputResponseMetadata{
...
func (b *shellOutputTool) executeRegexMode(rctx *rctx.ToolContext, params ShellOutputParams) (ToolResponse, error) {
...
func validateParams(params ShellOutputParams) error {
...
func fillProcessStatus(metadata *ShellOutputResponseMetadata, processID string, processes []*daemon.ProcessInfo) {
	for _, p := range processes {
		if p.ID == processID {
