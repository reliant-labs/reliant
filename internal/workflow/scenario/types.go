...
type SimulatedEvent struct {
	...
	//
	// It exists because the default is the opposite: a loop's body executes,
	// evaluating each inner node's config, so a broken expression fails the
	// scenario. Mocking a loop as a unit is sometimes exactly right; setting
	// this makes that a VISIBLE choice by the scenario author. The event's
	// output is the loop's aggregate output (`_results`, `_iterations`,
	// declared outputs, …) exactly as downstream nodes read it.
...
type Expectation struct {
	...
	//
	// The runner executes the real save_message resolution — condition,
	// templates, content-free guard — against each mocked result.
	//
	//	messages:
...
// ExecutionDetails records what a scenario run actually did.
type ExecutionDetails struct {
	...
	// SavedMessages lists every message the run saved, in order.
