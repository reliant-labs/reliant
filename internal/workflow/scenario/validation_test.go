...
// An event keyed by a ref URL ("builtin://agent") names a workflow, not a
// position in the graph — one URL can back many nodes. It is a validation
// error that points the author at the node path instead of an event that
// silently matches every node referencing that workflow.
func TestValidateScenario_RefURLEventKeyIsRejected(t *testing.T) {
	workflow, err := ParseWorkflowYAML([]byte(`
name: ref-key
entry: [run_checks]
nodes:
  - id: run_checks
...
`))
	...
	result := ValidateScenario(&Scenario{
		Name:   "ref_keyed",
		Events: []SimulatedEvent{{Node: "builtin://agent", Output: map[string]interface{}{"response_text": "x"}}},
	}, workflow)

	require.False(t, result.Valid)
	require.Len(t, result.Errors, 1)
	assert.Contains(t, result.Errors[0], "is a workflow ref, not a node")

	result = ValidateScenario(&Scenario{
		Name:   "path_keyed",
		Events: []SimulatedEvent{{Node: "run_checks", Output: map[string]interface{}{"response_text": "x"}}},
	}, workflow)
