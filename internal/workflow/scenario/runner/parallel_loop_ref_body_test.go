...
// builtin/parallel-compete.yaml's parallel_compete_use_winner, the runner
// reached only [improve_prompt, save_improved_prompt,
// implementations.create_wt, review]. Read literally, that says the real
// runtime enters the
...
//     when the scenario mocked its internals. The gate now opens a ref whose
//     internals a scenario targets (see transparentRefs).
...
func TestUnmockedRefStaysBlackBoxed(t *testing.T) {
	...
	transparent := transparentRefs(wf, sc.Events, LoadBuiltinWorkflow)
