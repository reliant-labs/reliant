...
//
// These tests are why scenarios run on the real runtime: a graph-walking
// simulator mocked `approval` as an ordinary activity and reported both shapes
// green, so neither defect was visible from a scenario.
//
...
func runYAMLScenario(t *testing.T, yamlStr string, sc *scenario.Scenario) *Result {
	t.Helper()
