	...
	for i, event := range scenario.Events {
		if event.Node != "" {
			if strings.Contains(event.Node, "://") {
				// A ref URL names a workflow, not a position in this graph: one
				// URL can back many nodes, so an event keyed by it has no single
				// node to answer. Address the node that references it instead.
				result.AddError("event[%d]: %q is a workflow ref, not a node — key the event by the node path that references it (e.g. the loop or workflow node id)",
					i, event.Node)
			} else if !isValidNodeRef(event.Node, validNodes) {
...
// ToolCallMsg.input is `string`. Catching it here means a bad mock is rejected
// where it is written, with a message naming the field, instead of failing
// deep inside save_message's convertToToolCalls.
//
...
// AnalyzeFalsePasses inspects a COMPLETED run and reports the ways it could
// have passed green without testing what the scenario author thinks it tested.
// These are warnings, never errors: the pattern is sometimes exactly what the
// author wants.
//
// Black-box sub-workflow: a ref-mode `workflow` node with no
// internally-qualified events is replaced wholesale by a scenario literal, so
// the entire sub-workflow — every node in it — never runs.
//
// (An unmocked router needs no warning: the runner answers it with an empty
// decision, and the run fails exactly as production does.)
func AnalyzeFalsePasses(scenario *Scenario, workflow *reliantv1.Workflow, exec *ExecutionDetails, loader WorkflowRefLoader) []string {
...
// never executed. Inline sub-workflows always execute, so they are exempt.
//
...
// BlackBoxedNodes returns the node paths a scenario mocks as a unit with
// `black_box: true`.
func BlackBoxedNodes(events []SimulatedEvent) map[string]bool {
	return explicitBlackBoxNodes(events)
}

// explicitBlackBoxNodes returns the nodes the scenario opted out of body
// execution for with `black_box: true`, so deliberate opacity is silent while
// accidental opacity still warns.
...
// scenario's qualified event node refs — the same prefixes the runner uses to
// decide whether to execute a referenced sub-workflow's body.
