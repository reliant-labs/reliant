...
func TestCallLLMActivityInput_LoopContextFields(t *testing.T) {
	...
	t.Run("proto node JSON with call_llm oneof is deserialized correctly", func(t *testing.T) {
		...
		protoJSON := `{
			...
			"node": {
				...
				"call_llm": {
					...
					"tools_config": {"preloaded_tools": {"literal": {"values": ["view", "edit"]}}}
				}
			}
		}`
		...
		require.Equal(t, []string{"view", "edit"}, input.Node.GetCallLlm().GetToolsConfig().GetPreloadedTools().GetLiteral().GetValues())
	})
}
