...
func runInline(t *testing.T, workflowYAML, scenarioYAML string) *Result {
	...
	scenarios, err := simulator.ParseScenarioYAML([]byte(scenarioYAML))
	require.NoError(t, err)
...
`)
	assert.Equal(t, simulator.StatusPassed, res.Status, "%v", res.Mismatches)
...
// A raw mock may write tool_calls[].input as a map; the wire shape is a JSON
// string. The runner encodes it so save_message's tool_calls field resolves.
func TestRunner_RawToolCallInputMapIsEncoded(t *testing.T) {
	res := runInline(t, `
name: tool-save
entry: [ask]
nodes:
...
expect:
  outcome: error
`)
	assert.Equal(t, simulator.StatusPassed, res.Status,
