...
// =============================================================================
// YIELD CONDITION EVALUATION (via typed workflow CEL API)
// =============================================================================

func TestYieldConditionViaV2celAPI(t *testing.T) {
	t.Run("agent yield: inputs.yield true", func(t *testing.T) {
		inputs := map[string]interface{}{"yield": true, "max_turns": int64(200)}
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 1}, Outputs: map[string]interface{}{}, Inputs: inputs}

		result, err := wfcel.EvaluateTemplate("{{inputs.yield}}", ctx)
		require.NoError(t, err)
		assert.Equal(t, true, result, "yield=true should return true")
	})

	t.Run("agent yield: inputs.yield false", func(t *testing.T) {
		inputs := map[string]interface{}{"yield": false, "max_turns": int64(200)}
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 1}, Outputs: map[string]interface{}{}, Inputs: inputs}

		result, err := wfcel.EvaluateTemplate("{{inputs.yield}}", ctx)
		require.NoError(t, err)
		assert.Equal(t, false, result, "yield=false should return false")
	})

	t.Run("yield with CEL expression", func(t *testing.T) {
		outputs := map[string]interface{}{"tool_calls": []interface{}{map[string]interface{}{"id": "t1"}}}
		...
		result, err := wfcel.EvaluateTemplate("{{inputs.yield && size(outputs.tool_calls) > 0}}", ctx)
		require.NoError(t, err)
		assert.Equal(t, true, result, "yield with tool_calls check should be true")
	})

	t.Run("yield with iteration check", func(t *testing.T) {
		outputs := map[string]interface{}{}
		inputs := map[string]interface{}{"yield": true}
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 0}, Outputs: outputs, Inputs: inputs}

		result, err := wfcel.EvaluateTemplate("{{inputs.yield && iter.iteration == 0}}", ctx)
		require.NoError(t, err)
		assert.Equal(t, true, result)
	})

	t.Run("yield with boolean input", func(t *testing.T) {
		inputs := map[string]interface{}{"yield": true}
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 0}, Outputs: map[string]interface{}{}, Inputs: inputs}
	...
	// Edge cases
	// -------------------------------------------------------------------------
	t.Run("empty expression returns empty string", func(t *testing.T) {
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 0}, Outputs: map[string]interface{}{}, Inputs: map[string]interface{}{}}
		result, err := wfcel.EvaluateTemplate("", ctx)
		require.NoError(t, err)
		assert.Equal(t, "", result, "empty yield expression should return empty string")
	})

	t.Run("string true coerces via template", func(t *testing.T) {
		inputs := map[string]interface{}{"yield": "true"}
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 0}, Outputs: map[string]interface{}{}, Inputs: inputs}

		result, err := wfcel.EvaluateTemplate("{{inputs.yield}}", ctx)
		require.NoError(t, err)
		assert.Equal(t, "true", result, "string 'true' should be returned as string")
	})

	t.Run("string false coerces via template", func(t *testing.T) {
		inputs := map[string]interface{}{"yield": "false"}
		ctx := &wfcel.LoopEvalContext{Iter: &model.IterContext{Iteration: 0}, Outputs: map[string]interface{}{}, Inputs: inputs}

		result, err := wfcel.EvaluateTemplate("{{inputs.yield}}", ctx)
		require.NoError(t, err)
		assert.Equal(t, "false", result, "string 'false' should be returned as string")
	})
}
