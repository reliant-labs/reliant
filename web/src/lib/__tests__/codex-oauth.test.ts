...
describe("runCodexOAuthFlow", () => {
  ...
  it("calls daemon, validates state, and exchanges code", async () => {
    ...
    const parsedUrl = new URL(
      capturedUrlTemplate.replace("{redirect_uri}", "http://placeholder"),
    );
    ...
    expect(parsedUrl.searchParams.get("id_token_add_organizations")).toBe(
      "true",
    );
    expect(parsedUrl.searchParams.get("codex_cli_simplified_flow")).toBe(
      "true",
    );
  ...
  it("returns daemon_error when daemon RPC throws", async () => {
    startOAuthViaDaemonMock.mockRejectedValue(
