...
![Presets allow you to quickly switch between different agent behaviors](/images/screenshots/presets-list.png)

When you select the "researcher" preset, it configures the agent with a specialized system prompt for research tasks, lower temperature for focused analysis, and a curated set of read-oriented tools. You get a research-optimized agent without touching individual parameters.

Reliant includes built-in presets for common development tasks: general coding, research, code review, documentation, refactoring, debugging, and more. You can also create custom presets in `.reliant/presets/`. For the complete list and customization options, see the [Presets](/workflows/presets) guide.
