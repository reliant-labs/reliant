...
stateDiagram-v2
    [*] --> Running
    Running --> Paused: Esc / Stop button
    Running --> Paused: Step failure
    Paused --> Running: Resume / Send message
    Paused --> Paused: Update parameters
    Paused --> Paused: Message sub-agent thread
    Paused --> Paused: Branch conversation
```
