...
| reproducer | claude-4.5-sonnet | view, edit, tag:search, tag:shell, shell_list, shell_output, fetch | Runtime bug reproduction. Discovers entry points, adds logging, captures evidence. |
...
| git | claude-4.5-sonnet | tag:shell, view, tag:search | Version control operations. Conventional commits, atomic changes. Loads the `conflict-resolver` skill for complex merge conflicts. |
...
| ux | claude-4.5-sonnet | fetch, glob, grep, view, write, edit, tag:shell, get_plan, list_tasks, update_task, component_library | UI design and implementation. WCAG 2.1 AA compliant, various style modes. |
