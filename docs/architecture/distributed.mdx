...
flowchart TB
    ...
    subgraph Cloud["Cloud Infrastructure"]
        LB[Load Balancer]
        subgraph API["API Servers (stateless, N replicas)"]
            API1[API Server 1]
            API2[API Server N]
        end
        subgraph Workers["Workers (stateless, N replicas)"]
            W1[Worker 1]
            W2[Worker N]
        end
        subgraph Gateways["Gateways (stateful, few replicas)"]
            GW1[Gateway 1]
        end
        NATS[(NATS)]
        PG[(Postgres)]
